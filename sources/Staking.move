...
module publisher::adf_stake {
    ...
    public entry fun createNewStake<CoinType>(admin: &signer) {
        ...
        move_to<Treasury<CoinType>>(admin, Treasury<CoinType>{staking: coin::zero<CoinType>(), totalStaking: 0, stakeInfo: table::new(), apy: 2000, refBonus: 500});
    }

    public entry fun getLatestAPY<CoinType>(admin: &signer):u128  acquires Treasury{
        let admin_addr = signer::address_of(admin);
        assert!(admin_addr == MODULE_ADMIN, ERROR_NOT_CREATOR);
        assert!(exists<Treasury<CoinType>>(admin_addr), ERROR_COIN_NOT_INITIALIZED);
        let treasury = borrow_global<Treasury<CoinType>>(MODULE_ADMIN);
        treasury.apy
    }
    ...
    public entry fun stake<CoinType>(sender: &signer, stakeAmount: u128) acquires Treasury{
        ...
        if(hasRef) {
            vector::push_back(&mut refs, StakeInfo{amount: stakeAmount, startTime:  timestamp::now_seconds(), duration: 10 * 24 * 60 *60, apy: treasury.apy, refBonus: treasury.refBonus, claimedAmount: 0});
        } else {
            vector::push_back(&mut refs, StakeInfo{amount: stakeAmount, startTime:  timestamp::now_seconds(), duration: 10 * 24 * 60 *60, apy: treasury.apy, refBonus: 0, claimedAmount: 0});
        };
        table::add(&mut treasury.stakeInfo, senderAddress, refs);
    }

    public entry fun claim<CoinType>(sender: &signer, stakeId: u64) acquires Treasury{
        let senderAddress = signer::address_of(sender);
        let treasury = borrow_global_mut<Treasury<CoinType>>(MODULE_ADMIN);
        ...
        let refs = table::borrow_mut(&mut treasury.stakeInfo, senderAddress);
        let stakeInfo = vector::borrow_mut<StakeInfo>(refs, stakeId);
        ...
        assert!(stakeReward > stakeInfo.claimedAmount, ERROR_NO_REWARD);
        stakeReward = stakeReward - stakeInfo.claimedAmount;
        ...
        stakeInfo.claimedAmount = stakeInfo.claimedAmount + stakeReward;
    ...
    public entry fun unstake<CoinType>(sender: &signer, stakeId: u64) acquires Treasury{
        ...
        assert!(stakedTime > 0, ERROR_STAKETIME_TOOSMALL);
        let stakeReward = stakeInfo.amount * stakeInfo.apy / Divider * (stakedTime as u128)/ SECOND_PER_YEAR;
        if(stakeReward > stakeInfo.claimedAmount) {
            stakeReward = stakeReward - stakeInfo.claimedAmount;
            ...
            stakeReward = stakeReward + stakeInfo.amount;
        } else {
            stakeReward = stakeInfo.amount;
        };
        let stakeRewardCoin = coin::extract<CoinType>(&mut treasury.staking, (stakeReward as u64));

        coin::deposit<CoinType>(signer::address_of(sender), stakeRewardCoin);
    }
    public fun getStake<CoinType>(sender: &signer): vector<StakeInfo> acquires Treasury{
        let senderAddress = signer::address_of(sender);
        let treasury = borrow_global_mut<Treasury<CoinType>>(MODULE_ADMIN);
        assert!(table::contains(&mut treasury.stakeInfo, senderAddress), EACCOUNT_NOT_FOUND);
    ...
    public(friend) fun test_test(aptos_framework: signer, origin_account: signer, acount2 :signer) acquires Treasury {    
        ...
        assert!(getLatestAPY<ADF>(&origin_account) == 2000, 0);
        ...
        claim<ADF>(&acount2, 0);
        assert!(coin::balance<ADF>(signer::address_of(&acount2))== 200000000000, coin::balance<ADF>(signer::address_of(&acount2)));
